...
class TestCatalogFilters:
    ...
    @pytest.mark.asyncio
    async def test_list_catalog_no_filters(
    ...
    @pytest.mark.asyncio
    async def test_list_catalog_language_filter(
        self, roadmap_service: RoadmapService
    ):
    ...
    @pytest.mark.asyncio
    async def test_list_catalog_tag_filter(
        self, roadmap_service: RoadmapService
    ):
    ...
    @pytest.mark.asyncio
    async def test_list_catalog_min_views_filter(
        self, roadmap_service: RoadmapService
    ):
    ...
    @pytest.mark.asyncio
    async def test_list_catalog_min_syncs_filter(
        self, roadmap_service: RoadmapService
    ):
    ...
    @pytest.mark.asyncio
    async def test_list_catalog_combined_filters(
        self, roadmap_service: RoadmapService
    ):
...
class TestCatalogSorting:
    ...
    @pytest.mark.asyncio
    async def test_sort_by_most_viewed(
        self, roadmap_service: RoadmapService
    ):
        ...
        if len(results) > 1:
            ...
            for i in range(len(results) - 1):
                ...
                view_count_next = results[i + 1].repo_summary.get(
                    "view_count", 0
                )
                assert view_count_i >= view_count_next

    @pytest.mark.asyncio
    async def test_sort_by_most_synced(
        self, roadmap_service: RoadmapService
    ):
        ...
        if len(results) > 1:
            ...
            for i in range(len(results) - 1):
                ...
                sync_count_next = results[i + 1].repo_summary.get(
                    "sync_count", 0
                )
                assert sync_count_i >= sync_count_next

    @pytest.mark.asyncio
    async def test_sort_by_highest_rated(
        self, roadmap_service: RoadmapService
    ):
        ...
        if len(results) > 1:
            ...
            for i in range(len(results) - 1):
                rating_count_i = results[i].repo_summary.get(
                    "rating_count", 0
                )
                ...
                rating_count_next = results[i + 1].repo_summary.get(
                    "rating_count", 0
                )
                rating_sum_next = results[i + 1].repo_summary.get(
                    "rating_sum", 0
                )
                avg_rating_next = (
                    rating_sum_next / rating_count_next
                    if rating_count_next > 0
                    else 0
                )
...
class TestCatalogPagination:
    ...
    async def test_pagination_second_page(
        ...
        # Get second page
        second_page, _ = await roadmap_service.list_catalog(
    ...
    async def test_pagination_total_count_consistency(
        ...
        _, total_count_page1 = await roadmap_service.list_catalog(
            page=1, page_size=10
        )
        _, total_count_page2 = await roadmap_service.list_catalog(
            page=2, page_size=10
        )
...
class TestCatalogEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_min_rating_boundary(
        self, roadmap_service: RoadmapService
    ):
    ...
    @pytest.mark.asyncio
    async def test_filter_with_sorting(
        self, roadmap_service: RoadmapService
    ):
