...
class GeminiChatService:
        ...
        if stage_id:
            ...
            if stage:
                ...
                if commit_window and len(commit_window) == 2:
                    commits = self._find_commits_for_window(repo_full_name, commit_window)
                    context += "\nRelevant Commits/Code:\n"
                    for commit in commits:
                        # Truncate content to avoid blowing up context
                        content_snippet = (commit.content or "")[:1000]
                        context += f"Commit {commit.commit_sha[:7]}: {content_snippet}\n\n"
            else:
    ...
    def _find_commits_for_window(self, repo_full_name: str, window: list[str]) -> list[RepoCommitChunk]:
        ...
        for chunk in all_chunks:
            if chunk.commit_sha.startswith(start_sha) or start_sha.startswith(chunk.commit_sha):
                in_window = True
            
            if in_window:
                found.append(chunk)
            
            if chunk.commit_sha.startswith(end_sha) or end_sha.startswith(chunk.commit_sha):
                in_window = False
                break
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code >= 400:
                    ...
                    raise GeminiGenerationError(f"Gemini API error: {response.status_code}")
