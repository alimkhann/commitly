...
class PolarService:
    ...
    def __init__(self) -> None:
        ...
        use_sandbox = settings.polar_sandbox_enabled or server_flag in {
            "sandbox",
            "true",
            "1",
            "yes",
        ...
        if use_sandbox:
            ...
            success_url = (
                settings.polar_sandbox_success_url or settings.polar_success_url
            )
            server = "sandbox"
            config_name = "sandbox"
        else:
            ...
            for name, value in (
                (
                    (
                        "POLAR_ACCESS_TOKEN"
                        if not use_sandbox
                        else "POLAR_SANDBOX_ACCESS_TOKEN"
                    ),
                    access_token,
                ),
                (
                ...
                (
                    (
                        "POLAR_SUCCESS_URL"
                        if not use_sandbox
                        else "POLAR_SANDBOX_SUCCESS_URL"
                    ),
                    success_url,
                ),
            )
        ...
        if missing_fields:
            ...
            raise PolarConfigurationError(
                f"Polar {config_name} configuration is incomplete. Missing: {joined}."
            )
    ...
    def create_checkout(
        self, amount_cents: Optional[int] = None, email: Optional[str] = None
    ) -> dict:
        payload: dict = {
            "products": [self._product_id],
            "success_url": self._success_url,
        }
        # For pay-what-you-want, do not include amount.
        # If provided, validate range (50..99_999_999)
