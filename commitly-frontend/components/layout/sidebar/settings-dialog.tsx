...
export default function SettingsDialog({ open, onOpenChange }: SettingsDialogProps) {
  const [activeTab, setActiveTab] = useState<(typeof SECTION_CONFIG)[number]["id"]>("general")
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl border-none bg-transparent p-0 shadow-none">
        <div className="flex h-[620px] overflow-hidden rounded-3xl border border-border/60 bg-card shadow-2xl shadow-black/40">
            ...
            <div className="flex-1 overflow-y-auto p-6 text-sm">
              ...
              {activeTab === "notifications" && (
                <NotificationsSection
                  ...
                  setWeeklyDigestEnabled={setWeeklyDigestEnabled}
                />
              )}
            </div>
          </div>
